...
M = 63000;                    %Количество итераций для метода Монте-Карло
P = 0;                        %Мощность
...
NumberOfCars = 100;
NumberOfAllCars = 500;       %Общее число машин в районе
N = NumberOfCars*2;          %Число поездок всех электромобилей в этот день
Pnapr = 0;
Pmoshn = 0;
Perc = 0;                   %Процент электромобилей участвующих в V2G
PowerPotr = 0.05;          %Потребляемая мощность одной зарядной станции (МВт)
PowerVyd = 0.05;          %Выдаваемая мощность в V2G одним электромобилем (МВт)
...
for i=1:(kusl-1)
    Y(i, k)=sum(datasample(Prob,N));             %кол-во подключенных машин в пик нагрузки
    Z(i, k)=sum(datasample(IfSoc,N))/N;           %процент машин с достаточным уровнем зарядки для V2G
